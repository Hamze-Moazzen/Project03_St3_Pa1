%% Plot bifurcation curves versus control parameter variation (for orthogonal axis)
...
A = 1.7758; % equal to the preference parameter (gamma)
B = 1.0;    % always 1
g = 1.0290; % mutual inhibition parameter
...
initial_state_forward = [1.3326; 0.0];
...
% Calculate the alpha values for plotting:
alpha_for_A1 = alpha_forward(t);
alpha_for_A2 = fliplr(alpha_backward(t));

% Plot A1 from the forward sweep (A1 is increasing)
plot(alpha_for_A1, flipud(A1_forward), 'LineWidth', 3, 'Color', 'b');
hold on;

% Plot A2 from the backward sweep (A2 is decreasing)
plot(alpha_for_A2, A2_backward, '-.', 'LineWidth', 3, 'Color', 'r');
hold off;

% title('A_1 & A_2 vs. α (Cardinal Axis)', 'FontSize', 16);
title('Amplitudes vs. Control Parameter (α)', 'FontSize', 16);
xlabel('Control Parameter (α)','FontSize',14,'FontWeight','bold');
ylabel('A_1, A_2 (a.u.)','FontSize',14,'FontWeight','bold');
grid on;
legend({'A_1: 90°','A_2: 0°'},'FontSize',12);
